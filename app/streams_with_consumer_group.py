...
# Estanblish connection to Redis
...
first_mesage = {"field1": "value1", "field2": "value2"}

# Add the first message with the custon Message Id
first_message = {"field1": "value1", "field2": "value2"}
r.xadd(stream_key, first_message)
...
group_exists = any(group == group_name for group in consumer_groups_info)
if group_exists:
    print(f"consumer group '{group_name}' exists.")
else:
    print(f"Consumer group '{group_name}' does not exits.")
    create = r.xgroup_create(stream_key, group_name, id="0", mkstream=True)
    print(f"Consumer group '{group_name}' create: {create}")
...
# Read message from the stream within the consumer group
stream_entries = r.xreadgroup(group_name, consumer_name, {stream_key: ">"}, count=10)
for stream, message in stream_entries:
    for message_id, fields in message:
        print(f"Message ID: {message_id}, Fields: {fields}")
        acknowleged = r.xack(stream_key, group_name, message_id)
        print(f"Message '{message_id}' acknowlegend: {acknowleged}")
